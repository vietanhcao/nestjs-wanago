...
import { ApproveActionDto, ApprovedDto, RejectedDto } from './dto';
...
export class ServiceApproveController {
  ...
  async rejected(
    ...
    await this.approveModel.rejected(_body);
    return Resolve.ok(0, 'Success');
  }
  ...
  /**
   * Lắng nghe yêu cầu từ chối phê duyệt tạo category
   * @param dto
   * @returns
   */
  @OnEvent(ApproveConfigs[ApproveActions.CREATE_CATEGORY].rejected, {
    async: true,
    promisify: true, // thêm option này để có thể await throw exception
  })
  createCategoryRejected(dto: ApproveActionDto) {
    console.log('Message Received: ', dto);
    return this.approveCategory.createCategoryRejected(
      dto,
      ApproveActions.CREATE_CATEGORY,
    );
  }
